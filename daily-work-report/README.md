...
支持两台 Mac 错开定时生成：先拉取另一台的日记，再新增或更新本机区块，最后按授权提交推送。重复运行不会重复追加，另一台区块和手写内容保留。

在各机已有的 `~/.daily-work-report/config.json` 中保留原配置，只补充：

```json
{
  "device_id": "personal-mac",
  "device_name": "个人 Mac",
  "legacy_device_id": "personal-mac"
}
```

公司 Mac 将前两项改为 `work-mac`、`公司 Mac`。这是新增字段示例，不是完整配置；两台必须分别设置本机 output_dir 和已授权 sources。旧无设备标记的日记若属于个人 Mac，两台均设置 `legacy_device_id: personal-mac`；没有旧区块可省略，未知归属不能猜测。

启用后 save 自动保存到 `日记根目录/2026/9/12.md` 这样的年/月/日路径，每台一个独立自动区块，按设备 → 状态栏目 → 项目分组；单机原有格式不受影响。重跑旧日期时，仅旧区块所属设备进行迁移。

生成前运行（路径和远程地址替换为用户已授权值）：

```sh
python <script> sync --repo <笔记仓库根目录> --remote-url <origin地址> --branch main
```

成功后再 collect、审阅、生成草稿及 save；save 使用原配置，脚本内部会先验证标准日报，再安全更新设备区块。最后由宿主按照授权范围检查差异、提交和推送。工作区不干净、历史分叉或同步失败时停止，不自动覆盖未提交笔记；远程领先且可以快进则自动拉取。此功能不安装定时任务，需在两台宿主分别配置并错开时间。完整执行规则见 [SKILL.md](daily-work-report/SKILL.md#多设备共用日记显式开启)。
